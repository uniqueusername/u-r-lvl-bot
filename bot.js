...
var xpMessages = [" i think this is slavery but you have ", " u fuck u got ", " when do i get my paycheck, because it better be more than ", " i really am not getting paid for this, can i have some of that juicy ", " u could buy a car with this dank af ", " idk what youre smoking but thats a thicc ", " the snail sleeps for ", " dad said he went to the store but i havent seen him since the fall of ", " you are now allowed to identify as this many genders: ", " bush did "];
...
bot.on('message', msg => {
  ...
  if (userLevelStats["timerEnabled"] === false) {
    ...
    console.log(userLevelStats["messageArray"]);
...
bot.on('message', msg => {
  ...
  if (msg.content === "where is the cheeto nick") {
...
// xp resetter
bot.on('message', msg => {
  if (msg.content.match('reset xp of') != undefined && msg.author.id == '83807335650164736') {
    var userToReset = msg.content.split(' ')[3];
    var userLevels = JSON.parse(fs.readFileSync('userLevels.json'));
  userLevels[userToReset.slice(2, -1)] = 0;
  fs.writeFileSync('userLevels.json', JSON.stringify(userLevels), 'utf8');
  }
})
